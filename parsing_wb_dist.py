...
"""def smth(json_file):
    info = []
    for x in json_file['data']['products']:
        info
"""
...
def save_excel(data, filename):
    ...
    test.sort()
    #print(test)t
    plt.hist(x=test,bins = 100,density=True,color="green")
   # plt.tight_layout()
    plt.grid(True)
    plt.show()
...
def smth (data):
    ...
    for x in data["Цена"]:
        test_0.append(x)
    test_0.sort()
    name, p = scipy.stats.shapiro(test_0) # тест Шапиро-Уилк
    print('Statistics=%.3f, p-value=%.3f' % (name, p))
...
if __name__ == '__main__':
    ...
    url = 'https://www.wildberries.ru/catalog/igrushki/dlya-malyshey'
    low_price = 1000
    top_price = 300000
